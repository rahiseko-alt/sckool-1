...
**目的**: 60人規模の生徒が匿名アカウントで仮想企業を経営し、商品を作って売り買いしながら
マーケティングを実践する、6言語対応のビジネスシミュレーション・プラットフォームを作ること。
要件の正本は `docs/requirements.md`（第1部＝ユーザーの原文、第2部＝MVP 受け入れ基準 A〜K）。
土台には OSS の Mercur / Medusa を使い、MP 経済・テスト・広告・不正検知・匿名認証を足す。

このリポジトリは雛形 `from-0` から始まった。雛形自身の計画は `docs/plan.from-0.json` に
残してある（`docs/decisions.md` の「28.」までの T 番号はそちらを指す）。`docs/plan.json` は
このプロジェクトの計画で、T001 から振り直している。
