...
const listing = await request('POST', '/store/listings', {
  market_id: sellerAccount.body?.market_id,
...
const sellerView = await request(
  'GET',
  `/store/transactions?market_id=${sellerAccount.body?.market_id}`,
);
...
const low = await request('POST', `/store/quizzes/${quizSummary.id}/submit`, {
  market_id: other.body?.market_id,
