...
1. **判定役をもう一度呼ぶ**（このセッションで着手済み。3体を並行で走らせた）。
   結果は `/tmp/.../scratchpad/verdict-abc.md` `verdict-defg.md` `verdict-hijk.md` に集約させ、
   まとめて `docs/release-check.md` を書き換える。**この3ファイルはコンテナ破棄で消える**ので、
   `docs/release-check.md` に落とすまでが1セット
2. **未達が出たら直す。** 前回は7件落ちた。全部通ったら `T043` を `done` にして
   `/plan-verify`（全体照合）
3. `/checkout`

**判定役に渡した共通の説明は `scratchpad/judge-brief.md` に置いた（これも消える）。**
中身は「読んではいけないもの（`docs/plan.json`・git 履歴・前回の `docs/release-check.md`・
この引継ぎ）」「動いている環境の URL と公開鍵と管理者の資格情報」「この環境の落とし穴」の3つ。
次のセッションで作り直すなら同じ構成にすること。
...
**公開は計画（`docs/plan.json`）に入っていない。** `T043` を終えたあと、`plan:next-id` で
番号を取って追記するか、計画の外の作業として扱うかを、ユーザーに確認すること。
