...
- 計画は **`docs/plan.json` の44項目**（`T001`〜`T044`）。うち **42件が `done`**、
  残る `T043`（リリース判定）は **`awaiting_human`**。中身は済んでおり、
  **ユーザーが「リリース可」と言えば終わり**（`automation` が `human` のため）
...
**残っているのは `T043` の1件だけで、中身は済んでいる。**
`docs/release-check.md` に **33基準すべてを満たした**と記録済み（2回の通し照合で
合計15件の未達を見つけ、全部直した）。`automation` が `human` なので、
**ユーザーが「リリース可」と言えば `done` にできる**。`/plan-ask` で聞く。

そのあと `/plan-verify`（全体照合）を回す。
