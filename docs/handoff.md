...
- 計画は **`docs/plan.json` の44項目**（`T001`〜`T044`）。**2026-09-02 に全件 `done`。**
  最後の `T043`（リリース判定）は、**ユーザー本人が「完成としてよい」と判断**して閉じた
  （`automation` が `human` のため、判定役ではなく人が決める）
...
手順は `docs/deploy.md`。**まだ一度も実行していない**（この環境に資格情報が無い）。
...
**計画44項目は全て終わった（2026-09-02）。** `docs/release-check.md` に
**33基準すべてを満たした**と記録済み（2回の通し照合で合計15件の未達を見つけ、全部直した）。
最後の `T043` はユーザー本人が「完成としてよい」と判断して閉じた。

**次にやること**

1. **`/plan-verify`（全体照合）を回す。** ゴールに到達したので、全項目をコード実体と
   突き合わせる。**同時に `docs/neglected-log.md` を読む**（ゴール到達時の振り返りに使う決まり）
2. **公開作業**（下記）

**次の大きな作業は「公開」。** 手順は `docs/deploy.md`、根拠は `docs/decisions.md`「42.」。
**計画（`docs/plan.json`）に入っていない**ので、`plan:next-id` で番号を取って追記するか、
計画の外の作業として扱うかを、ユーザーに確認すること。

**判定役の呼び方（次に同じことをするとき）**: `docs/plan.json`・git 履歴・
前回の `docs/release-check.md`・この引継ぎを**読ませない**。動いているサーバーと画面だけを
触らせる。A〜C／D〜G／H〜K の3分割で並行させると1時間弱で終わる。
渡す説明は「読んではいけないもの」「動いている環境の URL と公開鍵と管理者の資格情報」
「この環境の落とし穴」の3つで足りた。

**2回とも「済」にした項目から落ちている**（1回目7件・2回目8件、重複なし）。
**3回目も落ちる前提で臨むこと。** 落ちた項目は毎回、別の角度から見たときに崩れている。

**「使えるか」への答えは、いまのところ「授業ではまだ」。** 中身は揃ったが
**どこにも公開していない**。**置き場所は決まった**（`docs/decisions.md`「42.」、手順は
`docs/deploy.md`）が、実際の作成には**ユーザー本人がアカウントを作る必要がある**
（Oracle の申し込みには本人確認のクレジットカードが要る。課金はされない）。
**この開発環境には資格情報が無いので、AI 側だけでは公開できない。**
